...
> **Cutmaster 적용 가이드 (프로젝트 전용)**
>
> 본 시스템은 [getdesign.md/linear.app/design-md](https://getdesign.md/linear.app/design-md)에서 가져온 Linear 디자인 시스템 원본이다. Cutmaster에 적용할 때는 아래 매핑을 따른다.
>
> - **테마**: Linear 원본은 다크 모드 네이티브이지만 Cutmaster는 라이트 테마가 기본이다. 본 문서의 라이트 모드 뉴트럴(섹션 2 "Light Mode Neutrals")을 표준으로 쓰고, 다크 토큰은 향후 다크 모드 도입 시 참고용으로 보존한다.
> - **폰트**: Cutmaster는 한글 UI이므로 Inter Variable 대신 이미 번들된 `Pretendard-Regular.ttf`를 본문 기본으로 유지한다. Linear의 **weight 510 / 590 / 400 3-tier 위계**, **음수 letter-spacing**, **타이트한 line-height**는 그대로 적용한다 (Pretendard도 Variable이라 호환됨). Berkeley Mono 자리에는 시스템 모노(`ui-monospace, SF Mono, Menlo`)를 그대로 쓴다 — 라이선스가 없는 폰트는 도입하지 않는다.
> - **액센트**: Linear의 indigo-violet (`#5e6ad2` / `#7170ff`)을 **유일한 chromatic accent**로 채택. 1차 CTA, 활성 탭, focus ring에만 사용한다. 자재별 색상 프리셋(캔버스의 사각형)은 사용자 데이터이므로 디자인 시스템 색이 그것을 가리지 않도록 한다.
> - **border / 표면**: 라이트 모드에서는 `#e6e6e6` / `#d0d6e0` 솔리드 보더와 `#f7f8f8` / `#f3f4f5` 표면 단계를 사용. 다크 모드 적용 시에는 본문에 정의된 semi-transparent white border 시스템을 그대로 사용한다.
> - **반경 스케일**: 섹션 5의 Border Radius Scale을 그대로 채택 — 버튼 6px, 카드 8px, 패널 12px, pill 9999px.
> - **밀도**: Cutmaster는 좌측 입력 패널 + 우측 캔버스/요약의 데이터 밀도 높은 도구이므로 Linear의 "compressed headlines, expanded surroundings" 원칙을 적용한다. 표/입력은 타이트하게, 섹션 간격은 넉넉하게.
>
> 새 위젯/화면을 만들 때 AI 에이전트는 이 문서를 단일 소스 오브 트루스로 삼아야 한다.
